...
export function App() {
  ...
  return <Dashboard rooms={rooms} user={user} token={token} onOpenRoom={(id) => navigate('room', id)} onCreated={async (id) => { await loadRooms(); navigate('room', id) }} onLogout={logout} />
}
...
/** Even sizes only: players are split evenly into Red and Blue. */
const ROOM_SIZES = [2, 4, 6, 8, 12, 16, 32]

function Dashboard({ rooms, user, token, onOpenRoom, onCreated, onLogout }: { rooms: Room[]; user: User; token: string; onOpenRoom: (id: string) => void; onCreated: (roomId: string) => Promise<void>; onLogout: () => void }) {
  const [showCreate, setShowCreate] = useState(false); const [error, setError] = useState(''); const [creating, setCreating] = useState(false)
  // Set synchronously: state alone lets two submits in the same tick both get through
  const submitting = useRef(false)
  const closeCreate = () => { if (!creating) { setShowCreate(false); setError('') } }
  const createRoom = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const form = new FormData(event.currentTarget)
    try {
      const room = await api<{ id: string }>('/rooms', { method: 'POST', body: JSON.stringify({ name: String(form.get('name') ?? '').trim(), maxMembers: Number(form.get('maxMembers')), battleType: 'flag steal' }) }, token)
      setShowCreate(false)
