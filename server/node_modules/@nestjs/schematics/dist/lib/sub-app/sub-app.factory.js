...
import { findNestCliConfigPath, inPlaceSortByKeys, normalizeToKebabOrSnakeCase, readJsonFile, } from '../../utils/index.js';
...
function readBuilder(host) {
    ...
    const config = path ? readJsonFile(host, path) : null;
    const builder = config?.compilerOptions?.builder;
    const name = typeof builder === 'string'
        ? builder
        : builder?.type;
    return typeof name === 'string' && name ? name : CLI_DEFAULT_BUILDER;
}
function isBundler(builder) {
...
function updateNpmScripts(scripts, options, defaultAppName) {
    ...
    if (scripts[defaultTestScriptName] &&
        ...
        scripts[defaultTestScriptName] = scripts[defaultTestScriptName].replace(/(^|[\s"'=])(?:(\.\/)test(?![\w.-])|test(?=\/))/g, `$1$2${newTestDir}`);
    }
...
function applyStartProdScript(options, defaultAppName) {
    return (host) => {
        if (!host.exists('package.json')) {
            return host;
        }
