...
import { EmptyError, isObservable, Observable, of, ReplaySubject, } from 'rxjs';
...
export class RouterExecutionContext {
    ...
    create(instance, callback, methodName, moduleKey, requestMethod, contextId = STATIC_CONTEXT, inquirerId) {
        ...
        const handler = (args, req, res, next, handlerObservable$) => async () => {
        ...
        return async (req, res, next) => {
            ...
            const handlerObservable$ = isSseHandler || isEmptyArray(interceptors)
                ? undefined
                : new ReplaySubject(1);
            const resultOrDeferred = this.interceptorsConsumer.intercept(interceptors, [req, res, next], instance, callback, handler(args, req, res, next, handlerObservable$), contextType);
            const result = isSseHandler ? resultOrDeferred : await resultOrDeferred;
            await fnHandleResponse(result, res, req, handlerObservable$);
        };
    }
    ...
    createHandleResponseFn(callback, isResponseHandled, redirectResponse, httpStatusCode) {
        ...
        return async (result, res, req, handlerObservable$) => {
            ...
            if (isObservable(result)) {
                ...
                const disconnect$ = (handlerObservable$ ?? of(null)).pipe(switchMap(() => this.fromClientDisconnect(req, res)), tap(() => (disconnected = true)));
                result = result.pipe(takeUntil(disconnect$));
            }
            try {
    ...
    notifyIfObservable(result, handlerObservable$) {
        if (isObservable(result)) {
            ...
            result.then(value => isObservable(value) && handlerObservable$.next(), () => undefined);
        }
    }
    /**
